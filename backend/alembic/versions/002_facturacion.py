...
def upgrade() -> None:
    tasaiva = sa.Enum('IVA_10', 'IVA_5', 'EXENTO', name='tasaiva')
    estadofactura = sa.Enum('pendiente', 'pagada', 'cancelada', 'vencida', name='estadofactura')
    condicionventa = sa.Enum('contado', 'credito', name='condicionventa')

    tasaiva.create(op.get_bind(), checkfirst=True)
    estadofactura.create(op.get_bind(), checkfirst=True)
    condicionventa.create(op.get_bind(), checkfirst=True)
