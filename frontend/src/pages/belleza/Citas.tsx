    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <div className="text-center py-10 text-gray-400">Sin citas programadas</div>
        ) : (
          citas.map((c) => (
            <div key={c.id} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4 shadow-sm">
              <div className="flex-1">
                ...
                {c.precio_cobrado !== null && (
                  <p className="font-medium text-gray-800">Gs. {c.precio_cobrado.toLocaleString()}</p>
                )}
              </div>
              <span className={`text-xs px-2 py-1 rounded-full font-medium capitalize ${ESTADO_COLOR[c.estado] ?? 'bg-gray-100'}`}>
                {c.estado.replace('_', ' ')}
              </span>
            </div>
          ))
